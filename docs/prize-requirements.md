...
|---|---|---|
| Built **from scratch** during the event | `packages/subgraph` | New subgraph authored during the event: `subgraph.yaml`, `schema.graphql`, `src/mappings/*.ts` indexing Vouch events (Job, Guarantee, Payout, Provider). Git history shows it created from scratch. |
...
| **USDC on Arc** | `packages/contracts` (Arc testnet) + `apps/agent` | All accounting uses the **6-decimal ERC-20** USDC interface on Arc; live balances shown in the UI; tx links to arcscan. |
...
| Confidential portion **meaningfully contributes to core functionality** | `packages/cre-workflow` + `packages/contracts` (receiver) | The enclave verdict `{jobId, regressed, amount}` is the **sole trigger** for the guarantee payout via the receiver's `onReport`. |
