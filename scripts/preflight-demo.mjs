#!/usr/bin/env node
// Pre-demo health check for the Vouch monorepo (plan WF / feature request).
//
...
// credentials are never a failure — this mirrors the repo's "explicit not-configured state" rule
// and never fakes success. Read-only: it makes RPC/HTTP probes, never a transaction.
//
// Usage:  pnpm demo:health           (loads process.env; source your .env first)
// Env consulted (all optional): ARC_RPC_URL, ARC_CHAIN_ID (default 5042002), VOUCH_CORE_ADDRESS,
//   QUOTE_BOND_ESCROW_ADDRESS, USDC_ADDRESS, SUBGRAPH_URL, SUBGRAPH_MAX_LAG_BLOCKS,
//   AGENT_URL, API_URL (or NEXT_PUBLIC_API_BASE_URL).
...
if (!arcRpc) {
  ...
  try {
    ...
    if (cid === wantChain) row("arc", "RPC + chainId", OK, `chainId ${cid}`);
...
// ---- services ------------------------------------------------------------------------
for (const [track, label, base, path] of [
  ["arc", "agent /health", env.AGENT_URL, "/health"],
  ["arc", "api /health", env.API_URL || env.NEXT_PUBLIC_API_BASE_URL, "/health"],
]) {
  if (!base) row(track, label, NA, "service URL unset");
  else row(track, label, (await httpGet(base.replace(/\/$/, "") + path)) ? OK : BAD, base);
}
