 ...
 * A branded type stops a raw bigint (block number, bps) being formatted as money by accident.
 */
...
const HASH_RE = /^0x[a-fA-F0-9]{64}$/u;
const ADDR_RE = /^0x[a-fA-F0-9]{40}$/u;
...
export function explorerTxLink(hash: string | null | undefined, confirmedOnChain: boolean): string | null {
  if (!confirmedOnChain || !hash || !HASH_RE.test(hash)) return null;
  return `${EXPLORER}/tx/${hash}`;
}

export function explorerAddressLink(address: string | null | undefined): string | null {
  if (!address || !ADDR_RE.test(address) || address.toLowerCase() === ZERO_ADDR) return null;
